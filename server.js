...
// Verificar contraseña del admin (sin exponerla)
...
// CREAR PRODUCTO - Con subida a Supabase Storage (VERSION CORREGIDA)
app.post('/api/admin/productos', AUTH, upload.single('imagen'), async (req, res) => {
    try {
        ...
        console.log('💾 Insertando en Supabase:', productoData);
        
        const { data, error } = await supabase.from('products').insert(productoData).select();
...
// ACTUALIZAR PRODUCTO - Con subida a Supabase Storage
...
// ELIMINAR PRODUCTO - También elimina la imagen de Storage
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📸 Imágenes subidas a Supabase Storage`);
});